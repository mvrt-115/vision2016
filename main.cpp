...
void drawBoundedRects(cv::Mat& src, double focalLen, int d, int h, int calib, int contoursThresh)
{
	...
	if (minRect.size() > 0)
		calcDistance(src, minRect[0], focalLen, d, h, calib);
    if (contours.size() > 0)
    {
...
void drawBoundedRects(cv::Mat& src, double focalLen, int d, int h, int calib, int contoursThresh)
{
 	...
 	cv::Mat img = cv::Mat::zeros(threshold_output.size(), CV_8UC3);
 	std::vector<std::vector<cv::Point> > contours;
 	std::vector<cv::Vec4i> hierarchy;
	...
	cv::cvtColor(src, threshold_output, CV_BGR2GRAY);
 	// Detect edges using Threshold
 	cv::threshold(threshold_output, threshold_output, contoursThresh, 255, cv::THRESH_BINARY);
	...
	// Bounded rectangle is the one at the 0th index
	if (minRect.size() > 0)
		calcDistance(img, minRect[0], focalLen, d, h, calib);
    if (contours.size() > 0)
    {
        c = corners(contours[0], img);
        calcHorizAngle(img, d, c, mc);
    }
    ...
    for (int i = 0; i < mc.size(); i++)
    {
        cv::circle(img, mc[i], 5, cv::Scalar(255, 255, 0));
    }
    // Draw the corners of the contour object
    for (int i = 0; i < c.size(); i++)
    {
        cv::circle(img, c[i], 5, cv::Scalar(0, 255, 0));
    }
    // Draw the contours
  	for (int i = 0; i < contours.size(); i++)
    {
